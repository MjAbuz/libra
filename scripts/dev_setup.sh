...
function install_cargo_profile {
  LINE="$( dirname $( which cargo ) )/../env"
  echo Adding rustup/cargo to .profile
  if ! grep -q "$LINE" "${HOME}/.profile"; then
    echo "$LINE" >> "${HOME}/.profile"
    echo Added rustup/cargo to .profile
  fi
  eval $LINE
}
...
[[ INSTALL_PROFILE == "false" ]] && install_cargo_profile
